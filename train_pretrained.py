...
def build_datasets(train_percentage=0.8, preproc=False):
    ...
    mel_dims = [0,224,224,3]#get_sample_dimensions(path=path)  # Find out the 'shape' of each data file
    ...
    for idx, classname in enumerate(class_names):
        ...
        for idx2, infilename in enumerate(class_files[0:n_load]):
            ...
            if (idx2 < n_train):
                ...
                X_train[train_count, :, :, 0] = melgram.resize(224,224)
                X_train[train_count, :, :, 1] = melgram.resize(224,224)
                X_train[train_count, :, :, 2] = melgram.resize(224,224)
                ...
                X_test[test_count, :, :,0] = melgram.resize(224,224)
                X_test[test_count, :, :,1] = melgram.resize(224,224)
                X_test[test_count, :, :,2] = melgram.resize(224,224)
...
def build_model( nb_classes):
    ...
    final = Dense(nb_classes, activation='softmax')(final)  # dva izlaza, female/male
...
if __name__ == '__main__':
    ...
    #           verbose=1, validation_data=(x_test, y_test), callbacks=[mc,es])

    score = model.evaluate(x_test, y_test, verbose=0)
    print('Test score:', score[0])
    print('Test accuracy:', score[1])
    ##### BACA ERROR ZBOG MANJKA MEMORIJE
